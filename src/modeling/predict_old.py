...
def build_csv_row(exp_name, results, correlation_summaries, latent_dim):
    ...
    for label in ("v_correct", "v_apparent", "v_traditionnal", "v_ref"):
        ...
        for key in ("fap_at_Pinj", "power_ratio", "n_sig_peaks_outside", "P_detected", "delta_P"):
    ...
    if "error" in mcmc:
        ...
        row["mcmc_acceptance_fraction"] = _to_float_safe(mcmc.get("acceptance_fraction"))
        ...
        if act_lat.size:
            ...
            for idx, name in enumerate(names):
                ...
                if vec is not None and vec.size:
                    row[f"{label}_corr_{name}_vs_latent_max_abs"] = float(np.max(np.abs(vec)))
                    row[f"{label}_corr_{name}_vs_latent_max_k"] = int(np.argmax(np.abs(vec)) + 1)
                    row[f"{label}_corr_{name}_vs_latent_mean_abs"] = float(np.mean(np.abs(vec)))
...
def main(experiment_name: str, device: str = "cuda", mcmc_steps: int = 1000, mcmc_burn: int = 500):
    ...
    plot_latent_distance_distribution(
        delta_s_rand, delta_s_aug, os.path.join(post_dir, "latent_distances_distribution.png")
    )
